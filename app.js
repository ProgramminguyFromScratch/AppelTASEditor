...
//     const camera = {
//         x: Math.max(0, playerState.PLAYER_X - frameRendererCanvas.width / 2),
//         y: Math.min(0, (frameRendererCanvas.height - playerState.PLAYER_Y) - frameRendererCanvas.height / 2)
//     };
...
// function parseInputSequence(str) {
...
// /**
//  * Parse frame-pin string like "1:WD, 3:D, 5:" into { 1: 9, 3: 1, 5: 0 }.
//  * Keys are 1-based frame indices; values are bitmasks (W=8 A=2 S=4 D=1).
//  */
...
//         const keys = t.slice(colon + 1).trim().toUpperCase();
//         if (isNaN(frameNum) || frameNum < 1) continue;
//         let mask = 0;
//         if (keys.includes('W')) mask |= 8;
//         if (keys.includes('A')) mask |= 2;
//         if (keys.includes('S')) mask |= 4;
//         if (keys.includes('D')) mask |= 1;
//         result[frameNum] = mask;
//     }
